...
def metadata_processing():
    ...
    rock = pandas.Series(merge_columns(column('rock',df),merge_columns(column('hard rock',df),column('soft rock',df))))
    classical = pandas.Series(merge_columns(column('clasical',df),merge_columns(column('classical',df),column('classic',df))))
    electronic = pandas.Series(merge_columns(column('electronic',df),merge_columns(column('electronica',df),column('electro',df))))
